...
#         return bce
    ...
    def get_loss_fn(X, y, predictor, P, bce_loss_fn, optimize_P=False):
        I = torch.eye(X_train.shape[1]).to(device)
        y_fixed = y.clone()
        y_fixed[y_fixed<0.9] = -1
        pred = predictor(X @ (I - P)).squeeze()
        loss = 1 - pred * y_fixed
        loss[loss<0] = 0.0
        loss = loss**2
    ...
    if num_labels == 2:
        ...
        #predictor = torch.nn.Sequential(torch.nn.Linear(X_train.shape[1], 1,bias=False), torch.nn.Sigmoid(),
        #                     torch.nn.Linear(1, 1, bias=True)).to(device)
